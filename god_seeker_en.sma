...
public PlayerBlind(const index, const inflictor, const attacker, const Float:fadeTime, const Float:fadeHold, const alpha, Float:color[3])
{
	...
	{	
		client_print_color(index, index, "^3You in ^4GOD SEEKER^3 and can't be blind!")
...
public show_seeker_menu(id)
{
	if (!GOD_SEEKER_ENABLE[id])
	{
		client_print(id, print_chat, "God Seeker mode DISABLED.");
		return PLUGIN_HANDLED;
	}
	
	new tmpmenuitem[128];
	
	format(tmpmenuitem,127,"[God Seeker] Settings:");
		
	new vmenu = menu_create(tmpmenuitem, "seeker_menu")

	format(tmpmenuitem,127,"\wINVISIBLE [\r%s\w]", 
	GOD_SEEKER_DISABLE_VISIBILITY[id] == 0 ? "DISABLED" :
	(GOD_SEEKER_DISABLE_VISIBILITY[id] == 1 ? "MODE 1" : 
	(GOD_SEEKER_DISABLE_VISIBILITY[id] == 2 ? "MODE 2" : 
	(GOD_SEEKER_DISABLE_VISIBILITY[id] == 3 ? "MODE 3" : 
	(GOD_SEEKER_DISABLE_VISIBILITY[id] == 4 ? "MODE 4" : 
	(GOD_SEEKER_DISABLE_VISIBILITY[id] == 5 ? "INVISIBLED MODEL" 
	: "DISABLED" ))))));
	menu_additem(vmenu, tmpmenuitem,"1")
	format(tmpmenuitem,127,"\wSound [\r%s\w]", GOD_SEEKER_DISABLE_SOUNDS[id] ? "NO HEAR" : "ENABLED");
	menu_additem(vmenu, tmpmenuitem,"2")
	format(tmpmenuitem,127,"\wUsername [\r%s\w]", GOD_SEEKER_DISABLE_AIMING[id] ? "NO USERNAME" : "DISPLAY");
	menu_additem(vmenu, tmpmenuitem,"3")
	format(tmpmenuitem,127,"\wDamage [\r%s\w]", GOD_SEEKER_DISABLE_DAMAGE[id] ? "INVUL" : "YES");
	menu_additem(vmenu, tmpmenuitem,"4")
	format(tmpmenuitem,127,"\wTeleport with knife [\r%s\w]", GOD_SEEKER_ENABLE_TELEPORT[id] ? "ACTIVATED" : "DEACTIVATED");
	menu_additem(vmenu, tmpmenuitem,"5")
	format(tmpmenuitem,127,"\wDisable");
	menu_additem(vmenu, tmpmenuitem,"6")


	
	menu_setprop(vmenu, MPROP_NEXTNAME, "\yNEXT")
	menu_setprop(vmenu, MPROP_BACKNAME, "\yPREV")
	menu_setprop(vmenu, MPROP_EXITNAME, "\rEXIT")
...
public seeker_menu(id, vmenu, item) 
{
	...
	{	
		case 1:
		{
			...
			{
				client_print(id,print_chat, "[God seeker] MODE 2. Not works with SMOKE GRENADE!!")
			...
			{	
				client_print(id,print_chat, "[God seeker] MODE 3!")
			...
			{
				client_print(id,print_chat, "[God seeker] MODE 4!")
			...
			else if (GOD_SEEKER_DISABLE_VISIBILITY[id] == 4)
			{
				...
				{
					client_print(id,print_chat, "[God seeker] MODE 5 INVISIBLED MODEL!")
					custom_player_models_set(id,"invisibled_model");
...
#endif
				{
					client_print(id,print_chat, "[God seeker] DISABLED!")
			...
			{
				client_print(id,print_chat, "[God seeker] MODE 1!")
...
public enable_god_seeker(id)
{
	...
	log_to_file( "god_seeker.log", "ADMIN %s ACTIVATE God Seeker.", username )
	if(is_user_connected(id))
	{
		client_print(id, print_chat, "[God seeker] You entered in God seeker. Settings /godmenu");
		print_bad_users();
	}
	if (GOD_SEEKER_USERS == 0)
	{
